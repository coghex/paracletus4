...
-- | vulkan specific code fot the pipeline and layout
module Vulk.Pipeline where
import Prelude()
import UPrelude ( ($), fromIntegral, Foldable(length), (∘), (⌄) )
import Graphics.Vulkan
import Graphics.Vulkan.Core_1_0
import Graphics.Vulkan.Ext.VK_KHR_swapchain
import Graphics.Vulkan.Marshal.Create
import Graphics.Vulkan.Marshal.Create.DataFrame ( setDFRef )
import Numeric.DataFrame ( Nat, Vector, scalar )
import Numeric.Dimensions
    ( KnownDim, Dimensions(dims), inSpaceOf, totalDim )
import Prog ( MonadIO(liftIO), Prog )
import Prog.Foreign ( allocaPeek )
import Prog.Util ( allocResource )
import Vulk.VulkData
    ( SwapchainInfo(SwapchainInfo, swapExtent, swapImgFormat) )
import Vulk.Foreign ( runVk, withVkPtr )

-- | one giant vulkan struct, would be a pain in the ass in c
createGraphicsPipeline ∷ KnownDim (n ∷ Nat) ⇒ VkDevice
  → SwapchainInfo → VkVertexInputBindingDescription
  → Vector VkVertexInputAttributeDescription n
  → [VkPipelineShaderStageCreateInfo] → VkRenderPass
  → VkPipelineLayout → VkSampleCountFlagBits → Prog ε σ VkPipeline
createGraphicsPipeline dev SwapchainInfo{swapExtent} bindDesc
  attrDescs shaderDescs renderPass pipelineLayout msaaSamples
  = allocResource
...
createPipelineLayout ∷ VkDevice → VkDescriptorSetLayout
  → Prog ε σ VkPipelineLayout
...
-- | another giant vulkan structure
createRenderPass ∷ VkDevice → SwapchainInfo → VkFormat
  → VkSampleCountFlagBits → Prog ε σ VkRenderPass
createRenderPass dev SwapchainInfo{swapImgFormat} depthFormat samples
  = allocResource (\rp → liftIO $ vkDestroyRenderPass dev rp VK_NULL)
