...
import Codec.Picture
    ( convertRGBA8,
      ...
      Image(Image, imageWidth, imageHeight, imageData),
      PixelRGBA8(..) )
...
import Vulk.Font ( FontTex(..), loadFTChar, TTFData(..) )
import Vulk.Buff ( createBuffer, findMemoryType )
...
loadNTexs ∷ VkPhysicalDevice → VkDevice → VkCommandPool
  → VkQueue → FilePath → Prog ε σ [(VkImageView, VkSampler)]
loadNTexs pdev dev cmdPool cmdQueue path = do
  imgPaths ← liftIO $ getDirectoryContents path
  loadNTex pdev dev cmdPool cmdQueue $ map (combine path)
    $ sort $ filter filterOutPathJunk imgPaths
...
--   in the given list
loadNTex ∷ VkPhysicalDevice → VkDevice → VkCommandPool → VkQueue
  → [FilePath] → Prog ε σ [(VkImageView, VkSampler)]
loadNTex _ _ _ _ [] = return []
loadNTex pdev dev cmdPool cmdQueue (path:paths) = do
  (imgView, mipLevels)
    ← createTextureImageView pdev dev cmdPool cmdQueue path
  sampler ← createTextureSampler dev mipLevels
  texs ← loadNTex pdev dev cmdPool cmdQueue paths
  return $ (imgView, sampler) : texs

-- | recursively call sing texture image view creation
createTextureImageViews ∷ VkPhysicalDevice → VkDevice
  → VkCommandPool → VkQueue → [FilePath]
  → Prog ε σ [(VkImageView, Word32)]
createTextureImageViews _    _   _       _        []
  = return []
...
-- | generates vulkan images for each printable ascii character
createFontImageViews ∷ VkPhysicalDevice → VkDevice → VkCommandPool
  → VkQueue → FilePath → Int → Prog ε σ [((VkImageView, Word32),TTFData)]
createFontImageViews pdev dev cmdPool cmdQueue fp px
  = mapM (createFontImageView pdev dev cmdPool cmdQueue fp px) ['!'..'~']

-- | loads a specified freetype font, generates
--   bitmaps, converts to vulkan image
createFontImageView ∷ VkPhysicalDevice → VkDevice → VkCommandPool
  → VkQueue → FilePath → Int → Char
  → Prog ε σ ((VkImageView, Word32),TTFData)
createFontImageView pdev dev cmdPool cmdQueue fp px char = do
  (FontTex w h buff,ttfData) ← liftIO $ loadFTChar fp char px
  ...
  runCommandsOnce dev cmdPool cmdQueue $ transitionImageLayout image
    VK_FORMAT_R8G8B8A8_UNORM Undef_TransDst mipLevels
  locally $ do
    (stagingMem, stagingBuf) ← createBuffer pdev dev bufSize
      VK_BUFFER_USAGE_TRANSFER_SRC_BIT
      (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT
      ⌄ VK_MEMORY_PROPERTY_HOST_COHERENT_BIT )
    stagingDataPtr ← allocaPeek $ runVk
      ∘ vkMapMemory dev stagingMem 0 bufSize VK_ZERO_FLAGS
    ...
    copyBufferToImage dev cmdPool cmdQueue stagingBuf
      image (fromIntegral imageWidth) (fromIntegral imageHeight)
  runCommandsOnce dev cmdPool cmdQueue $ generateMipmaps pdev image
    VK_FORMAT_R8G8B8A8_UNORM (fromIntegral imageWidth)
    (fromIntegral imageHeight) mipLevels
  imageView ← createImageView dev image
    VK_FORMAT_R8G8B8A8_UNORM VK_IMAGE_ASPECT_COLOR_BIT mipLevels
  return ((imageView, mipLevels),ttfData)

-- | loads a texture
createTextureImageView ∷ VkPhysicalDevice → VkDevice → VkCommandPool
  → VkQueue → FilePath → Prog ε σ (VkImageView, Word32)
createTextureImageView pdev dev cmdPool cmdQueue path = do
  ...
  runCommandsOnce dev cmdPool cmdQueue $ transitionImageLayout
    image VK_FORMAT_R8G8B8A8_UNORM Undef_TransDst mipLevels
  locally $ do
    (stagingMem, stagingBuf) ← createBuffer pdev dev bufSize
      VK_BUFFER_USAGE_TRANSFER_SRC_BIT
      (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT
...
-- | generates mipmaps so that you can zoom out arbitrarily far and vulkan
--   will change the texture being sampled to blend pixels, i have
--   a feeling this isnt really doing all that much, but its hard to tell
generateMipmaps ∷ VkPhysicalDevice → VkImage → VkFormat → Word32
  → Word32 → Word32 → VkCommandBuffer → Prog ε σ ()
generateMipmaps pdev image format width height mipLevels cmdBuf = do
  formatProps ← allocaPeek $ \propsPtr → liftIO
    $ vkGetPhysicalDeviceFormatProperties pdev format propsPtr
  let supported = getField @"optimalTilingFeatures" formatProps
                ⌃ VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT
   in when (supported ≡ VK_ZERO_FLAGS) $ logExcept VulkError ExVulk
  ...
  let barrier = barrierStruct (mipLevels - 1)
   ...
   in withVkPtr barrier $ \barrPtr → liftIO $ vkCmdPipelineBarrier
        cmdBuf VK_PIPELINE_STAGE_TRANSFER_BIT
        VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT
        VK_ZERO_FLAGS 0 VK_NULL 0 VK_NULL 1 barrPtr
  where
  ...
  createLvl (mipLevel, srcWidth, srcHeight) = do
    let barrier = barrierStruct (mipLevel - 1)
     ...
     in withVkPtr barrier $ \barrPtr → liftIO
          ...
          VK_PIPELINE_STAGE_TRANSFER_BIT
          VK_ZERO_FLAGS 0 VK_NULL 0 VK_NULL 1 barrPtr
    withVkPtr (blitStruct mipLevel srcWidth srcHeight) $ \blitPtr
      → liftIO $ vkCmdBlitImage cmdBuf image
      VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL image
      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL
      1 blitPtr VK_FILTER_NEAREST
    let barrier = barrierStruct (mipLevel - 1)
                    VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL
                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
                    VK_ACCESS_TRANSFER_READ_BIT
...
-- | recursively calls texture sampler creation
createTextureSamplers ∷ VkDevice → [Word32] → Prog e s [VkSampler]
...
-- | allcates a texture sampler, change if you want blurry textures
--   instead of the hard pixels it has now
createTextureSampler ∷ VkDevice → Word32 → Prog e s VkSampler
createTextureSampler dev mipLevels = do
  ...
  allocResource (liftIO ∘ flip (vkDestroySampler dev) VK_NULL)
    $ withVkPtr samplerCreateInfo $ \sciPtr → allocaPeek
    $ runVk ∘ vkCreateSampler dev sciPtr VK_NULL

-- | recursively returns image info for each texture
textureImageInfos ∷ [VkImageView] → [VkSampler]
  → Prog ε σ [VkDescriptorImageInfo]
...
-- | returns a static image layout for each sampler and image view
textureImageInfo ∷ VkImageView → VkSampler
  → Prog ε σ [VkDescriptorImageInfo]
...
-- | allcoates an image view in vulkan memory
createImageView ∷ VkDevice → VkImage → VkFormat
  → VkImageAspectFlags → Word32 → Prog ε σ VkImageView
createImageView dev image format aspectFlags mipLevels
  = allocResource (liftIO ∘ flip (vkDestroyImageView dev) VK_NULL)
  $ withVkPtr imgvCreateInfo $ \imgvciPtr → allocaPeek
  $ runVk ∘ vkCreateImageView dev imgvciPtr VK_NULL
...
-- | ADT for the image sampler, hlint does not like this, but its fine
data ImageLayoutTransition = Undef_TransDst
                           | TransDst_ShaderRO
                           | Undef_DepthStencilAtt
                           | Undef_ColorAtt

-- | vulkan transition data structure
...
-- | transition behavior depending on the image layout
...
-- | creates memory barrier between transitions
transitionImageLayout ∷ VkImage → VkFormat
  → ImageLayoutTransition → Word32 → VkCommandBuffer → Prog ε σ ()
transitionImageLayout image format transition mipLevels cmdBuf
  = withVkPtr barrier $ \barrPtr → liftIO $ vkCmdPipelineBarrier
    cmdBuf srcStageMask dstStageMask VK_ZERO_FLAGS
    0 VK_NULL 0 VK_NULL 1 barrPtr
  where TransitionDependent{..} = dependents transition
        aspectMask = case newLayout of
          VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
            | hasStencilComponent format → VK_IMAGE_ASPECT_DEPTH_BIT
...
-- | returns an image and the device memory where we have place an
--   image of a texture to be sampled in each individual vert calculation
createImage ∷ VkPhysicalDevice → VkDevice → Word32 → Word32 → Word32
  → VkSampleCountFlagBits → VkFormat → VkImageTiling → VkImageUsageFlags
  → VkMemoryPropertyFlags → Prog ε σ (VkDeviceMemory, VkImage)
createImage pdev dev width height mipLevels samples format
  ...
  imageMemory ← allocResource
    ...
    withVkPtr allocInfo $ \aiPtr → allocaPeek $ runVk
                        ∘ vkAllocateMemory dev aiPtr VK_NULL
  freeImageLater
  runVk $ vkBindImageMemory dev image imageMemory 0
  return (imageMemory, image)

-- | creates an image out of vkbuffer of memory
copyBufferToImage ∷ VkDevice → VkCommandPool → VkQueue
  → VkBuffer → VkImage → Word32 → Word32 → Prog ε σ ()
copyBufferToImage dev cmdPool cmdQueue buffer image width height
  = runCommandsOnce dev cmdPool cmdQueue $ \cmdBuf
  → withVkPtr region $ \regPtr → liftIO $ vkCmdCopyBufferToImage
  cmdBuf buffer image VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL 1 regPtr
...
-- | not all devices support linear tiling
findSupportedFormat ∷ VkPhysicalDevice → [VkFormat] → VkImageTiling
  → VkFormatFeatureFlags → Prog ε σ VkFormat
findSupportedFormat pdev candidates tiling features = do
  goodCands ← flip filterM candidates $ \format → do
    props ← allocaPeek $ \propsPtr → liftIO
      $ vkGetPhysicalDeviceFormatProperties pdev format propsPtr
...
-- | returns vkformat of tiling textures
findDepthFormat ∷ VkPhysicalDevice → Prog ε σ VkFormat
findDepthFormat pdev = findSupportedFormat pdev
  [VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT
  , VK_FORMAT_D24_UNORM_S8_UINT] VK_IMAGE_TILING_OPTIMAL
    VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT

-- | returns if there is a stencil in the format
hasStencilComponent ∷ VkFormat → Bool
hasStencilComponent format = format `elem`
  [VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT]

-- | depthattimgview is an entire image view
createDepthAttImgView ∷ VkPhysicalDevice → VkDevice → VkCommandPool
  → VkQueue → VkExtent2D → VkSampleCountFlagBits → Prog ε σ VkImageView
createDepthAttImgView pdev dev cmdPool queue extent samples = do
  depthFormat ← findDepthFormat pdev
  (_, depthImage) ← createImage pdev dev
    (getField @"width" extent) (getField @"height" extent)
      1 samples depthFormat
    VK_IMAGE_TILING_OPTIMAL VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT
    VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
  depthImageView ← createImageView dev depthImage depthFormat
    VK_IMAGE_ASPECT_DEPTH_BIT 1
  runCommandsOnce dev cmdPool queue $ transitionImageLayout
    depthImage depthFormat Undef_DepthStencilAtt 1
  return depthImageView

-- | colorattimgview is an entire image view
createColorAttImgView ∷ VkPhysicalDevice → VkDevice → VkCommandPool
  → VkQueue → VkFormat → VkExtent2D → VkSampleCountFlagBits
  → Prog ε σ VkImageView
createColorAttImgView pdev dev cmdPool queue format extent samples = do
  (_, colorImage) ← createImage pdev dev
    ...
    VK_IMAGE_TILING_OPTIMAL (VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT
      ...
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
  colorImageView ← createImageView dev colorImage format
    VK_IMAGE_ASPECT_COLOR_BIT 1
  runCommandsOnce dev cmdPool queue $ transitionImageLayout
    colorImage format Undef_ColorAtt 1
