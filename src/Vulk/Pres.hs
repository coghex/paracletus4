...
-- | Pres means presentation, swapchain and surface creation
module Vulk.Pres where
import Prelude()
import UPrelude
import Data.Maybe (fromMaybe)
import Data.Semigroup
    ( Arg(..), ArgMin, Min(Min, getMin), Option(Option, getOption) )
...
import Vulk.Data
    ( DevQueues(graphicsQueue, presentQueue, qFamIndices),
      SwapchainSupportDetails(..),
      VulkResult(VulkError) )
import Vulk.Foreign ( asListVk, runVk, withVkPtr )

-- | a surface is the logical screen area before swapchains
createSurface ∷ VkInstance → GLFW.Window → Prog ε σ VkSurfaceKHR
createSurface vkInstance window = allocResource
  (\s → liftIO $ vkDestroySurfaceKHR vkInstance s VK_NULL)
    $ allocaPeek $ runVk ∘ GLFW.createWindowSurface
    vkInstance window VK_NULL

-- | use the best swap surface format
chooseSwapSurfaceFormat ∷ SwapchainSupportDetails
  → Prog ε σ VkSurfaceFormatKHR
chooseSwapSurfaceFormat SwapchainSupportDetails {..}
  = maybe (logExcept VulkError ExVulk "No available surface formats!")
  (pure ∘ argVal ∘ getMin) ∘ getOption
...
chooseSwapPresentMode SwapchainSupportDetails {..}
  = argVal ∘ getMin ∘ fromMaybe (Min $ Arg 0 VK_PRESENT_MODE_FIFO_KHR)
  ∘ getOption $ foldMap (Option ∘ Just ∘ pmCost) presentModes
  where argVal (Arg _ b) = b
        pmCost ∷ VkPresentModeKHR → ArgMin Int VkPresentModeKHR
        pmCost VK_PRESENT_MODE_MAILBOX_KHR
          = Min $ Arg 0 VK_PRESENT_MODE_MAILBOX_KHR
        pmCost VK_PRESENT_MODE_IMMEDIATE_KHR
          = Min $ Arg 1 VK_PRESENT_MODE_IMMEDIATE_KHR
        pmCost VK_PRESENT_MODE_FIFO_KHR
          = Min $ Arg 2 VK_PRESENT_MODE_FIFO_KHR
        pmCost pm = Min $ Arg 3 pm

-- | set the width and height of the swapchain images
...
-- | allocate the swapchain and corresponding images
createSwapchain ∷ VkDevice → SwapchainSupportDetails
  → DevQueues → VkSurfaceKHR → Prog ε σ SwapchainInfo
createSwapchain dev scsd queues surf = do
  ...
  let maxIC = getField @"maxImageCount" $ capabilities scsd
      ...
      swCreateInfo = createVk @VkSwapchainCreateInfoKHR
          ...
          &* set @"preTransform"
                    (getField @"currentTransform" $ capabilities scsd)
